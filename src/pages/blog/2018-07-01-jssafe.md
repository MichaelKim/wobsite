...
The easiest way to notice this is to find all instances of the character `x` (function name) in the source code. The argument `х` isn't selected! Another thing to notice is the argument isn't (directly) used in the function. So the value of `h(str(x))` remains the same for any password.
